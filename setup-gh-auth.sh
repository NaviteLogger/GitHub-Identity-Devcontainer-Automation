...
# Export GITHUB_TOKEN from /workspace/.env so `gh` and git (via the gh credential
...
if [ -r /workspace/.env ]; then
  _GITHUB_TOKEN_FROM_ENV=$(awk -F= '/^GITHUB_TOKEN=/ { sub(/^GITHUB_TOKEN=/, ""); print; exit }' /workspace/.env)
